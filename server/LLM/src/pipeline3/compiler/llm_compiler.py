    ...
    def _parse_joinner_output(self, raw_answer: str) -> str:
        ...
        for ans in raw_answers:
            if ans.startswith("Thought:"):
                ...
                if JOINNER_FINISH in ans or JOINNER_REPLAN in ans:
                    # Begin capturing the answer
                    capture_answer = True
                    is_replan = JOINNER_REPLAN in ans
                    # Track the opening parenthesis count
                    open_paren_count += ans.count("(")
                    answer_parts.append(ans[ans.find("(") + 1:])
            elif capture_answer:
                # Continue capturing while parentheses are unbalanced
    ...
    async def _acall(
        ...
        for i in range(self.max_replans):
            ...
            if self.planner_stream:
                ...
                await self.message_manager.send_message("Analyzing query")
                ...
                await self.message_manager.send_message("Processing data")
            ...
            if not is_replan:
                ...
                if answer != NO_ANWER_REPLY:
                    ...
                    log_task_execution(self.user_id, self.ticker_name, self.other_mentioned_ticker_names, tasks, answer)
                break
