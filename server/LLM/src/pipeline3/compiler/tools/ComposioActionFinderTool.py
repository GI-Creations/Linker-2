...
class ComposioActionFinderTool:
    ...
    async def execute_action(self, userQuery: str, *args) -> str:
        ...
        try:
            action_enums=self.composio_tool_set.find_actions_by_use_case(App.SLACKBOT, use_case=userQuery, advanced=False)
    ...
    def get_tool(self):
        ...
        slack_action_finder_tool = StructuredTool.from_function(
            func=self.execute_action,
            name="slack_action_finder", 
            description=(
                "slack_action_finder(userQuery: str) -> dict:\n"
                " - Finds the most appropriate actions for userQuery.\n"
                " - Example usage:\n"
                "   slack_action_finder(USER_QUERY)\n"
        ...
        return slack_action_finder_tool