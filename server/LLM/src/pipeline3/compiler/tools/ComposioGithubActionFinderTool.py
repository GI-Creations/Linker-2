...
class ComposioGithubActionFinderInput(BaseModel):
    userQuery: str = Field(..., description="The userQuery to find the actions for")
    

class ComposioGithubActionFinderTool:
    ...
    async def execute_action(self, userQuery: str, *args) -> str:
        ...
        try:
            action_enums=self.composio_tool_set.find_actions_by_use_case(App.GITHUB, use_case=userQuery, advanced=False)
    ...
    def get_tool(self):
        ...
        github_action_finder_tool = StructuredTool.from_function(
            func=self.execute_action,
            name="github_action_finder", 
            description=(
                "github_action_finder(userQuery: str) -> dict:\n"
                " - Finds the most appropriate actions for userQuery.\n"
                " - Example usage:\n"
                "   github_action_finder(USER_QUERY)\n"
                " - Returns a list of enums response with action results.\n"
                " - Always use before ComposioTool for getting the github app action to execute and invoke ComposioTool after this always.\n"
                " - Use for Applications operations when needed, avoid otherwise."
            ),
            args_schema=ComposioGithubActionFinderInput,
        )
        return github_action_finder_tool